...
const defaultContact = {
  address: "Beirut, Lebanon · Abidjan, Ivory Coast",
  phones: [
  ...
  emails: ["Ezzedinep@hotmail.com", "abssobdeir@gmail.com"],
  instagram_handle: "@ezztravell",
  instagram_link: "https://www.instagram.com/ezztravell",
};

export default async function Footer() {
  const { data } = await supabase.from("contact_content").select("*").eq("id", 1).maybeSingle();

  const address = data?.address || defaultContact.address;
  const phones = data?.phones?.length ? data.phones : defaultContact.phones;
  const emails = data?.emails?.length ? data.emails : defaultContact.emails;
  const instagramHandle = data?.instagram_handle || defaultContact.instagram_handle;
  const instagramLink = data?.instagram_link || defaultContact.instagram_link;
    ...
    <footer id="contact" className="relative border-t border-edge/10 bg-surface px-6 pb-10 pt-16 text-fg">
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-10 md:flex-row md:justify-between">
        <div className="flex flex-col gap-3">
          ...
          <p className="text-sm text-fg/60">{address}</p>
        </div>
      ...
      <p className="mx-auto mt-3 max-w-6xl text-center text-[11px] text-fg/40">
        Website done by{" "}
        <a
